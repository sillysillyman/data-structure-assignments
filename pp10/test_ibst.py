...
class TestImmutableBST(unittest.TestCase):
    ...
    def test_e_range(self):
        ...
        for i in range(100):
            ...
            self.checkRange(
                t, els, els[lowi] - 0.4, els[highi] + 0.4, els[lowi : highi + 1]
            )
            self.checkRange(
                t, els, els[lowi] - 0.4, els[lowi] + 0.4, els[lowi : lowi + 1]
            )
    ...
    def test_g_split_time(self):
        ...
        self.assertLess(
            t1 - t0, 0.01, msg="Your Split does not seem to run in O(h) time."
        )
